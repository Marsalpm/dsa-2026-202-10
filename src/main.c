...
int main() {
  ...
  int mode;
  while (true) {
    printf("Com vols introduir la posició? ((1)address / (2)coordinate / "
           "(3)place / (0)exit): ");
    ...
    if (mode == 0)
      ...
      snprintf(filename, sizeof(filename), "maps/%s/streets.txt",
               mapname); // també carreguem carrers
      ...
      while (places == NULL) {
        ...
        printf("Introdueix el nom d'un mapa correcte (ex: xs_1, xs_2, "

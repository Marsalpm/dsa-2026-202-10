...
PlaceNode* findPlace(PlaceNode *head, char *name) {
 ...
 void cerca_inteligent_places(PlaceNode *head, char *name) {
    ...
    while (temp != NULL) {
        ...
        // Si el place actual té una distància de Levenshtein més petita que alguna de les opcions es fa intercanvi, sempre ordenat de més petita a més gran
        if (d < d1) {
            d3 = d2; strcpy(sug3, sug2);
            d2 = d1; strcpy(sug2, sug1);
            d1 = d; strcpy(sug1, temp->place.name);
        }
        else if (d < d2) {
            d3 = d2; strcpy(sug3, sug2);
            d2 = d; strcpy(sug2, temp->place.name);
        }
        else if (d < d3) {
            d3 = d; strcpy(sug3, temp->place.name);
        }
    ...
    int opcions = 0;
    //Imprimir les opcions si aquestes tenen una distància de Levenshtein més petita que 100
    if (d1 < 100) { printf("1. %s\n", sug1); opcions++; }
    if (d2 < 100) { printf("2. %s\n", sug2); opcions++; }
    if (d3 < 100) { printf("3. %s\n", sug3); opcions++; }

    // Si no s'ha imprès ninguna vol dir que no s'ha trobat ningún carrer similar, per tant ho imprimim 
    if (opcions == 0) {

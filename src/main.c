...
int main() {
  ...
  printf("\n\n--- ORIGIN ---\n");
  while (true) {
    printf("How do you want to input the position? ((1)address / (2)place / "
           "(3)coordinate / (0)exit): ");
    ...
    if (mode == 0) {
      ...
      int success = cerca_inteligent_houses(houses, carrer, numero, streets,
                                            &initial_lat, &initial_lon, 1);
      if (success == 1)
      ...
      scanf(" %99[^\n]", place);
      int success =
          cerca_inteligent_places(places, place, streets, &initial_lat,
                                  &initial_lon, 1); // funció de cerca
      // sortim del bucle
      if (success == 1)
        break;
      printf("\n");
      ...
      StreetNode *closest =
          findClosestStreet(streets, initial_lat, initial_lon);
      double dist = haversine(initial_lat, initial_lon, closest->street.lat1,
                              closest->street.lon1);
      while (dist > 10.0) {
        ...
        dist = haversine(initial_lat, initial_lon, closest->street.lat1,
                         closest->street.lon1);
      }
      printf("\n");
  ...
  printf("\n\n--- DESTINATION ---\n");
  while (true) {
    printf("Where do you want to go? Address (1), Place (2), Coordinate (3) or "
           "Exit (0): ");
    int valid_number = scanf("%d", &mode);
    while (valid_number != 1 || mode < 0 || mode > 3) {
      while (getchar() != '\n')
    ...
    if (mode == 0) {
      ...
      int success = cerca_inteligent_houses(houses, destination, number,
                                            streets, &final_lat, &final_lon, 0);
      if (success == 1)
        break;
      printf("\n");
    } else if (mode == 2) {
      ...
      printf("Enter the place name: ");
      scanf(" %99[^\n]", place);
      int success = cerca_inteligent_places(places, place, streets, &final_lat,
                                            &final_lon, 0); // funció de cerca
      // sortim del bucle
      if (success == 1)
        break;
      printf("\n");
      ...
      double dist = haversine(final_lat, final_lon, closest->street.lat1,
                              closest->street.lon1);
      while (dist > 10.0) {
        printf("Coordinates out of range, please try again\n");
        printf("Lat: ");
        ...
        dist = haversine(final_lat, final_lon, closest->street.lat1,
                         closest->street.lon1);
      }
      printf("\n");
  ...
  printf("\n\n--- ROUTE ---\n");
  Street fromStreet =
      findClosestStreet(streets, initial_lat, initial_lon)->street;
